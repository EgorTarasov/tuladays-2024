...
const getCreationDateById = `-- name: GetCreationDateById :one
...
func (q *Queries) GetCreationDateById(ctx context.Context, id int32) (User, error) {
	row := q.db.QueryRow(ctx, getCreationDateById, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Email,
